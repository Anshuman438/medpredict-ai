...
def create_random_model(n_classes):
    model_type = random.choice(["rf", "xgb", "et", "lgbm"])
    if model_type == "rf":
    ...
    def fit(self, X, y):
        ...
        for gen in range(self.generations):
            ...
            for i, ind in enumerate(population):
                ...
                model_names = [type(m).__name__ for m in ind.models]
                print(f"  Individual {i+1}: Acc={ind.fitness:.4f} | F1={ind.f1:.4f} | Models={model_names}")

            population.sort(key=lambda x: x.fitness, reverse=True)
            best_gen = population[0]
            self.history.append({
                "generation": gen + 1,
                "best_accuracy": round(best_gen.fitness, 4),
                "best_f1": round(best_gen.f1, 4)
            })
            print(f"\n  ✅ Best this gen: Acc={best_gen.fitness:.4f} | F1={best_gen.f1:.4f}")
        ...
        print(f"\n🏆 Final Best: Accuracy={self.best.fitness:.4f} | F1={self.best.f1:.4f}")
